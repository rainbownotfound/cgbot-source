...
function react(message, remaining, allowedMappings) {
    ...
    if(!allowedMappings[char]) return;
...
module.exports.run = (client, message, args) => {
        if(args.length < 1) {
            message.reply('You must provide some text to react with.');
        }

        const fetchOptions = { limit: 1 };
        if(args[1]) {
            if(!/\d{18}/.test(args[1]))
                return message.reply(`**${args[1]}** is not a valid message ID!`);
        ...
        message.channel.fetchMessages(fetchOptions).then(messages => {
            if(messages.length < 1)
                return message.channel.send('Failed to find the message.');
            ...
            console.log(`${chalk.cyanBright(`[React]`)} ${chalk.redBright(`${message.author.username}#${message.author.discriminator}`)} ${chalk.yellowBright(`made me react to a message!`)}\n${chalk.greenBright(args[0])}`);
        }).catch(message.error);
};
